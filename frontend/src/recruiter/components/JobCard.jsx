...
export default function JobCard({ job, onPublish, onClose, onViewApplicants, onEdit, onDelete }) {
  ...
  return (
    <div className="relative rounded-[30px] border border-white/10 bg-white/[0.04] p-6 transition duration-300 hover:-translate-y-1 hover:border-violet-400/25">
      <div className="mb-4 flex items-start justify-between">
        ...
        <div className="absolute right-6 top-6 flex flex-col items-end gap-2">
          ...
          <div className="mt-1 flex gap-2">
            ...
            <button onClick={() => { if (window.confirm("Are you sure you want to delete this job?")) onDelete(job.id); }} className="text-slate-500 transition-colors hover:text-rose-200" title="Delete Job">
      ...
      <p className="mb-4 pr-2 text-sm text-slate-400 line-clamp-3">{job.description}</p>
