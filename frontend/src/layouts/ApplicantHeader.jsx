...
const PAGE_TITLES = {
  "/applicant":              "Dashboard",
  "/applicant/resume":       "Resume Analysis",
  "/applicant/builder":      "Resume Builder",
  "/applicant/jobs":         "Browse Jobs",
  "/applicant/applications": "My Applications",
};

export default function ApplicantHeader({ onToggleSidebar, isDark, onToggleTheme }) {
  ...
  const initials = user?.email ? user.email.slice(0, 2).toUpperCase() : "AP";
  const displayName = user?.email ?? "Applicant";
  const pageTitle = PAGE_TITLES[location.pathname] ?? "Applicant Portal";
          ...
          {profileOpen && (
            ...
            >
              ...
              <div className="apl-dropdown-header">
                ...
                <div className="apl-dropdown-header-role">{user?.role ?? "applicant"}</div>
              </div>
