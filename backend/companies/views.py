...
class CompanyCreateView(APIView):
    ...
    def post(self, request):
        ...
        # Prevent recruiter who already has a company from creating another
...
class AddHRView(APIView):
    ...
    def post(self, request):
        if not (request.user.role == "admin" or getattr(request.user, "is_hr", False)):
...
class RemoveHRView(APIView):
    ...
    def post(self, request):
        ...
        if user.company != request.user.company and request.user.role != "admin":
...
class HRListView(APIView):
    ...
    def get(self, request):
        ...
        hrs = User.objects.filter(company=request.user.company, is_hr=True)
        data = [{"id": hr.id, "email": hr.email, "username": hr.username} for hr in hrs]
...
class CompanyMembersView(APIView):
    ...
    def get(self, request):
        ...
        members = User.objects.filter(company=user.company)
